...
// opcodes
// Added TIME opcode before EXIT
enum { LEA ,IMM ,JMP ,JSR ,BZ  ,BNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PSH ,
       OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,
       OPEN,READ,CLOS,PRTF,MALC,FREE,MSET,MCMP,TIME,EXIT }; 
  ...
  while ((tk = *p)) {
    ...
    if (tk == '\n') {
      if (src) {
        ...
        lp = p;
        while (le < e) {
          // Added TIME opcode to debug array
          printf("%8.4s", &"LEA ,IMM ,JMP ,JSR ,BZ  ,BNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PSH ,"
                           "OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,"
                           "OPEN,READ,CLOS,PRTF,MALC,FREE,MSET,MCMP,TIME,EXIT,"[*++le * 5]);
...
void expr(long long lev)
{
  ...
  if (!tk) { printf("%lld: unexpected eof in expression\n", line); exit(-1); }
    ...
    *++e = IMM; *++e = (ty == CHAR) ? sizeof(char) : sizeof(long long);
    ty = INT;
  }
  else if (tk == Id) {
    d = id; next();
    ...
    *++e = PSH;
    *++e = IMM; *++e = (ty > PTR) ? sizeof(long long) : sizeof(char);
    *++e = (t == Inc) ? ADD : SUB;
    *++e = (ty == CHAR) ? SC : SI;
  }
  ...
  while (tk >= lev) { // "precedence climbing" or "Top Down Operator Precedence" method
    ...
    if (tk == Assign) {
      ...
      *d = (long long)(e + 3); *++e = JMP; d = ++e;
      expr(Cond);
      *d = (long long)(e + 1);
    }
    else if (tk == Lor) { next(); *++e = BNZ; d = ++e; expr(Lan); *d = (long long)(e + 1); ty = INT; }
    else if (tk == Lan) { next(); *++e = BZ;  d = ++e; expr(Or);  *d = (long long)(e + 1); ty = INT; }
      ...
      next(); *++e = PSH; expr(Mul);
      if ((ty = t) > PTR) { *++e = PSH; *++e = IMM; *++e = sizeof(long long); *++e = MUL;  }
      *++e = ADD;
    }
    else if (tk == Sub) {
      next(); *++e = PSH; expr(Mul);
      if (t > PTR && t == ty) { *++e = SUB; *++e = PSH; *++e = IMM; *++e = sizeof(long long); *++e = DIV; ty = INT; }
      else if ((ty = t) > PTR) { *++e = PSH; *++e = IMM; *++e = sizeof(long long); *++e = MUL; *++e = SUB; }
      else *++e = SUB;
    }
      ...
      *++e = PSH; *++e = IMM; *++e = (ty > PTR) ? sizeof(long long) : sizeof(char);
      *++e = (tk == Inc) ? ADD : SUB;
      *++e = (ty == CHAR) ? SC : SI;
      *++e = PSH; *++e = IMM; *++e = (ty > PTR) ? sizeof(long long) : sizeof(char);
      *++e = (tk == Inc) ? SUB : ADD;
      ...
      if (t > PTR) { *++e = PSH; *++e = IMM; *++e = sizeof(long long); *++e = MUL;  }
...
int main(int argc, char **argv)
{
  ...
  // Added TIME keyword mapping
  p = "char else enum if int return sizeof while "
      "open read close printf malloc free memset memcmp time exit void main";
  ...
  if (!(pc = (long long *)idmain[Val])) { printf("main() not defined\n"); return -1; }
  if (src) return 0;
  ...
  *--sp = (long long)t;
  ...
  while (1) {
    i = *pc++; ++cycle;
    if (debug) {
      // Added TIME to debug print string
      printf("%lld> %.4s", cycle,
        &"LEA ,IMM ,JMP ,JSR ,BZ  ,BNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PSH ,"
         "OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,"
         "OPEN,READ,CLOS,PRTF,MALC,FREE,MSET,MCMP,TIME,EXIT,"[i * 5]);
      if (i <= ADJ) printf(" %lld\n", *pc); else printf("\n");
    }
    if      (i == LEA) a = (long long)(bp + *pc++);                             // load local address
    else if (i == IMM) a = *pc++;                                         // load global address or immediate
    else if (i == JMP) pc = (long long *)*pc;                                   // jump
    else if (i == JSR) { *--sp = (long long)(pc + 1); pc = (long long *)*pc; }        // jump to subroutine
    else if (i == BZ)  pc = a ? pc + 1 : (long long *)*pc;                      // branch if zero
    else if (i == BNZ) pc = a ? (long long *)*pc : pc + 1;                      // branch if not zero
    else if (i == ENT) { *--sp = (long long)bp; bp = sp; sp = sp - *pc++; }     // enter subroutine
    else if (i == ADJ) sp = sp + *pc++;                                   // stack adjust
    else if (i == LEV) { sp = bp; bp = (long long *)*sp++; pc = (long long *)*sp++; } // leave subroutine
    ...
    else if (i == TIME) a = (long long)time((time_t *)*sp); // Added TIME handler
