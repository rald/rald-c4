...
int main(int argc, char **argv)
{
  ...
  // Check if a file argument is present; if so, open it. Otherwise, read from stdin.
  if (argc > 0) {
    ...
    len = 0;
    while (1) {
      i = read(STDIN_FILENO, p + len, poolsz - len - 1);
      if (i <= 0) break;
      len += i;
    }
    if (len <= 0) { printf("read() from stdin returned %lld\n", len); return -1; }
    p[len] = 0;
  }
